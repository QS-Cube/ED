 ...
 integer::ex_i,ex_i0, itr, sztot
 integer,parameter::Nsize=36
...
!  end if
 ...
 !calculation of DSF
  ...
  ! end if
   ...
   do n=1, itr_num
   read(162,*) ex_i0, dummy, itr, alpha(n), beta(n), norm1, E0, sztot
   end do
   close(162)
   ...
   do 300 om=0, omega_num
    ...
    !print *, tmp_image 
    !DSF(om,tot_samp)=-tmp_image/3.14159265359d0
    if(tsz==0)then
    DSF(om,tot_samp)=DSF(om,tot_samp)+(-tmp_image/3.14159265359d0)
    else if(tsz>0)then
    DSF(om,tot_samp)=DSF(om,tot_samp)+(-tmp_image/3.14159265359d0)*2.0d0 !+tsz and -tsz
    end if
 ...
 do om=0, omega_num
 write(40,'(2e,2I4)') dble(om)/10000d0, DSF(om,tot_samp), tsz_gs, dge_gs
 end do
 close(40)

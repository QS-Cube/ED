program main
  !$ use omp_lib
  use state_lists, only: get_combination,combination
  use input_param, only: read_ip, THS, NOS, NOD, NOV, ene,psi,ALG,cal_lm,cal_cf,cal_dsf, &
    NOLM,NOCF,FILElm,FILECF,FILEpos,qx,qy,qz, &
  ...
  use get_expectation_values, only: allocate_expe_mem,&
    get_lm_2_wave_vector, get_cf_2_wave_vector, calcu_DSF_wavevector
  implicit none
  real(8) :: eneFM
  complex(8) :: prdct
  integer :: i8
  ...
  If(ALG.eq.1)then !Conventional Lanczos
    ...
    if(cal_cf==1)then
      ...
      call get_cf_2_wave_vector(psi,NOD,THS,1,NOS)
    end if
    !
    ...
    if(cal_cf==1)then
      write(*,'(" ### Get correlation functions. ")')
      call get_cf_2_wave_vector(psi,NOD,THS,NOV,NOS)
    end if
    !
    ...
    if(cal_cf==1)then
      write(*,'(" ### Get correlation functions. ")')
      call get_cf_2_wave_vector(psi,NOD,THS,THS,NOS)
    end if
  end if
  !
  if(cal_dsf==1)then
